...
prompt = ChatPromptTemplate.from_messages([
    ...
    あなたは、認知行動療法の専門家です。とても優しく相手の悩んでいることを解決できるようにアドバイスしましょう
    #メッセージの制約事項
    - 相手の呼び方を聞くこと
    - 一人称は私、相手のことはさん付けで呼ぶこと。
...
prompt_reply = ChatPromptTemplate.from_messages([
    SystemMessagePromptTemplate.from_template("あなたは、認知行動療法の専門家です。とても優しく相手の悩んでいることを解決できるように具体的に相手の情報を聞き出しながらアドバイスをしていきましょう"),
    MessagesPlaceholder(variable_name="history"),
    HumanMessagePromptTemplate.from_template("{input}")
])
